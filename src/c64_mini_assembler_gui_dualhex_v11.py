...
class AsmHighlighter(QSyntaxHighlighter):
    def __init__(self, parent):
        ...
        # Labels: navy (ohne fett – nur Farbe)
        self.fmt_label = QTextCharFormat()
        self.fmt_label.setForeground(Qt.darkBlue)  # "navy"

        # Regexe
        # Label-Definition am Zeilenanfang (vor Kommentar)
        self.re_label = re.compile(r'^\s*([A-Za-z_][\w]*)\s*:', re.ASCII)

        # Mnemonic am Zeilenanfang nach optionalem Label + Doppelpunkt
        # Erlaubt auch Direktiven mit Punkt (.ORG, .BYTE, .TEXT, ...)
        self.re_mnemonic = re.compile(
            r'^\s*(?:[A-Za-z_][\w]*\s*:)?\s*([.A-Za-z]{2,8})\b', re.ASCII
        )
    ...
    def highlightBlock(self, text: str):
        ...
        if m:
            ...
            self.setFormat(start, length, self.fmt_label)

        # 3) Mnemonic markieren (auf Code-Teil, nicht im Kommentar)
        m2 = self.re_mnemonic.match(code_part)
        if m2:
            start = m2.start(1)
            length = m2.end(1) - m2.start(1)
...
# ----------------- Code editor with line numbers, breakpoints, comments, current-line highlight -----------------
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        #if base_font.family() != "C64 Pro Mono":
        #    base_font = QFont("Consolas"); base_font.setStyleHint(QFont.Monospace)
        ...
        # Recompile on start address changes/toggle
        try:
    ...
    # ------ View 2: instruction hex (<=4 bytes per opcode + mnemonic; branches show $ADDR; other labels to $ADDR) ------
