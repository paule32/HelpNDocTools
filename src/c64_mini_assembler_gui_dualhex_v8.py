...
class MainWindow(QMainWindow):
    ...
    def _refresh_hex_instr(self, rows: List[Tuple[str,List[int],str,int]]):
        ...
        for addr_str, bytes_list, mnemonic_str, src_lineno in rows:
            ...
            # Show full bytes for .TEXT / .ASC (no truncation), otherwise max 4 bytes
            if mnem_upper in (".TEXT", ".ASC"):
                bytes_tokens = [f"{b:02X}" for b in bytes_list]  # full
            else:
                bytes_tokens = [f"{b:02X}" for b in bytes_list][:4]  # capped

            # Pad to keep columns readable for short lines
            bytes_str = " ".join(bytes_tokens)
            if len(bytes_tokens) < 4:
                bytes_str = bytes_str.ljust(11)

            # Branches: show absolute $ADDR
            ...
            line = f"{addr_str:>4}  {bytes_str}  {display_mnemonic}"
