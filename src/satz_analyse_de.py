"""
Satzanalyse DE (Schritt 1+2)
- Schritt 1: Rechtschreibung + Groß-/Kleinschreibung (Heuristik)
- Schritt 2: Einfache Satzbauchecks (Satzanfang groß, Endzeichen, feste Wendung „nach Hause“, Verb an 2. Position Heuristik)
Nutzung:
    python satz_analyse_de.py "dein satz hier"
"""
import sys, re, json
...
def analyze_sentence_step1(sentence: str, vocab_or_df: Union[dict, pd.DataFrame]):
    # Accept either a prepared vocab dict or the raw DataFrame
...
def main():
    if len(sys.argv) < 2:
        print("Nutzung: python satz_analyse_de.py \"Dein Satz hier\"")
        sys.exit(0)
    sentence = sys.argv[1]
    ...
    print(json.dumps({"step1": step1, "step2": step2}, ensure_ascii=False, indent=2))
