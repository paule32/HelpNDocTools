...
def get_range_urls(session: requests.Session):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "System Error Codes (" in text and "--" in href:
            if not href.startswith("http"):
                if href.startswith("/"):
                    href = "https://learn.microsoft.com" + href
                else:
                    href = "https://learn.microsoft.com/en-us/windows/win32/debug/" + href
            links.append(href)
    seen = set()
    ordered = []
    for u in links:
...
if __name__ == "__main__":
    out = "win_errors_ENG_ALL.po" if len(sys.argv) < 2 else sys.argv[1]
    main(out)
