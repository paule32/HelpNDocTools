...
- Schritt 4: Weitere Satzbau-Heuristiken (doppelte Konjunktionen, einfache Kommahinweise, doppelte Negation)
- Schritt 5: Flexions-Erkennung (häufige Verbformen, einfache Nomen-Plurale) – reduziert False Positives
Nutzung:
    python satz_analyse_de_v3.py "dein satz hier"
"""
import sys, re, json
...
# Schritt 4: weitere Satzbau-Heuristiken
CONJ = {"und","oder","aber","denn","doch","sondern","weil","dass"}

NEG_1 = {"nicht","nie","niemals","nichts","niemand","nirgends"}
NEG_2 = {"kein","keine","keinen","keinem","keiner","keines","nichts","niemand","nirgendwo","nirgends"}
...
def analyze_sentence_step4_syntax(sentence: str):
    ...
    # Doppelte Konjunktionen (direkt hintereinander)
    for i in range(len(toks) - 1):
        if toks[i] in CONJ and toks[i+1] == toks[i]:
            issues.append({"type":"conjunction_repeat", "tokens":[toks[i], toks[i+1]], "msg": f'Doppelte Konjunktion „{toks[i]} {toks[i+1]}“. Kürzen.'})

    # Viele "und"/"oder" -> evtl. Komma prüfen
    count_und = toks.count("und")
    count_oder = toks.count("oder")
    ...
    # Doppelte Negation (einfach)
    for i, t in enumerate(toks):
        if t in NEG_1:
            for j in range(i+1, min(i+6, len(toks))):  # in Fenster von 5 Wörtern
                if toks[j] in NEG_2:
...
# Schritt 5: Flexions-Erkennung (Verben + Nomen)
...
def try_recognize_verb_form(token_cf: str, vocab: Dict[str, set]):
    # derive base by stripping a common ending and appending 'en'
    for suf in VERB_ENDINGS:
        if token_cf.endswith(suf) and len(token_cf) > len(suf):
            base = token_cf[:-len(suf)]
            cand = base + "en"
            if cand in vocab["verbs"] or cand in vocab["all"]:
                return cand
    return None

def try_recognize_noun_plural(token: str, vocab: Dict[str, set]):
    # if token appears capitalized, try plural stripping (very simple, no umlaut handling)
    if not token[:1].isalpha():
        return None
    # check both capitalized and lower for robustness
    t_cap = token[0].upper() + token[1:]
    t_cf = token.casefold()
    for suf in NOUN_PLURAL_ENDINGS:
        if t_cf.endswith(suf) and len(t_cf) > len(suf):
            base = t_cf[:-len(suf)]
            # check base as given (capitalize)
    ...
    for tok in toks:
        ...
        if t_cf not in vocab["all"]:
            ...
            if nbase:
                ...
                continue
    return {"original": sentence, "recognized": recognized, "note": "Naive Erkennung; unregelmäßige Formen können fehlen."}

def main():
    if len(sys.argv) < 2:
        print("Nutzung: python satz_analyse_de_v3.py \"Dein Satz hier\"")
        sys.exit(0)
    sentence = sys.argv[1]
    ...
    print(json.dumps({"step1": step1, "step2": step2, "step3": step3, "step4": step4, "step5": step5}, ensure_ascii=False, indent=2))
