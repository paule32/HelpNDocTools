...
class C64BasicParser:
    ...
    def to_python(self, parsed_program):
        ...
        for i, line_number in enumerate(all_line_numbers):
            ...
            function_code = [f"    def line_{line_number}(self):"]
            ...
            if matching_lines:
                ...
                if tokens and tokens[0][1] == "END":
                    function_code.append("        self.running = 2")
                ...
                # Übersetzung für PRINT-Befehl
                    ...
                    function_code.append(f"        self.console.win.print_line(\"{content}\")")
                    function_code.append(f"        self.console.win.gotoxy(self.xpos, self.ypos)")
                    function_code.append(f"        self.ypos += 1")
        ...
        python_code.append("        self.running = 0")
        python_code.append("        self.xpos = 1")
        python_code.append("        self.ypos = 1")
        
        python_code.append("        self.console = DOSConsole()")
        python_code.append("        self.console.win.print_line('holladiho')")
        ...
        python_code.append("    def update_progress(self, value):")
        python_code.append("        while True:")
        python_code.append("            func_name = f'line_{self.current_line}'")
        python_code.append("            if hasattr(self, func_name):")
        python_code.append("                self.console.win.gotoxy(self.xpos, self.ypos)")
        python_code.append("                self.ypos += 1")
        python_code.append("                method = getattr(self, func_name)")
        python_code.append("                self.current_line = method()")
        python_code.append("                if not self.current_line == None:")
        python_code.append("                    self.current_line += 1")
        python_code.append("            else:")
        python_code.append("                showError(f\"methode: {func_name} not found.\")")
        python_code.append("                if self.worker_thread.isRunning():")
...
class FileWatcherGUI(QDialog):
    ...
    def init_ui(self):
        ...
        self.help_tabs.setMinimumWidth(900)
        # help
        self.tab0_0 = QWidget()
        self.tab1_0 = QWidget()
        self.tab2   = QWidget()
