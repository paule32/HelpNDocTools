"""
Satzanalyse DE (v4) – konfigurierbare Regeln + erweiterte Heuristiken
...
- Schritt 3: Feste Wendungen (konfigurierbar, Standard: zu Hause, im Allgemeinen, im Voraus, in Ordnung, in Gefahr, in Frage)
- Schritt 4: Weitere Satzbau-Heuristiken (Konjunktionsdopplungen – inkl. mit Zwischenzeichen; Kommahinweis; doppelte Negation)
- Schritt 5: Flexions-Erkennung (Verbformen inkl. Partizip II; einfache Nomen-Plurale inkl. -ien→-ium Heuristik)
Optional: JSON-Konfigurationsdatei "satz_analyse_config.json" im selben Ordner.
Nutzung:
    python satz_analyse_de_v4.py "dein satz hier"
"""
...
def analyze_sentence_step4_syntax(sentence: str):
    ...
    # Doppelte Konjunktionen, erlaubt Zwischenzeichen (Komma/Strichpunkt) und optional ein Füllwort dazwischen
    # Muster: conj [ , ; : - ]? (ein Wort)? conj
    ...
    for i in range(len(toks)-1):
        if toks[i] in conj_set:
            ...
            # mit einem Wort Abstand (z. B. "und dann und")
    ...
    for i, t in enumerate(toks):
        if t in neg1:
            for j in range(i+1, min(i+7, len(toks))):  # etwas größeres Fenster
                if toks[j] in neg2:
    ...
    for tok in toks:
        ...
        if t_cf not in vocab["all"]:
            ...
            if nbase:
                ...
                continue
    return {"original": sentence, "recognized": recognized, "note": "Heuristisch; unregelmäßige Formen/Umlaute teils unberücksichtigt."}

def main():
    if len(sys.argv) < 2:
        print("Nutzung: python satz_analyse_de_v4.py \"Dein Satz hier\"")
        sys.exit(0)
    sentence = sys.argv[1]
