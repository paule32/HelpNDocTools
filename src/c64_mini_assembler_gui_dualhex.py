...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("C64 Mini Assembler — Dual Hex Views")
        ...
        # Update views on resize to adapt text width in hexview_main
    ...
    # ------ View 2: instruction hex (<=4 bytes per opcode + mnemonic) ------
    def _refresh_hex_instr(self, listing: List[str]):
        out = ["ADDR  BYTES(max4)    MNEMONIC", "-"*48]
        for line in listing:
            try:
                ...
                for idx, tok in enumerate(parts):
                    if re.fullmatch(r"[0-9A-Fa-f]{2}", tok):
                        ...
                        if len(byte_tokens) >= 4:
                            mnem_start_idx = idx + 1
                            break
                    else:
                        mnem_start_idx = idx
                        break
                mnem_tokens = parts[mnem_start_idx:] if mnem_start_idx < len(parts) else []
                bytes_str = " ".join(byte_tokens)
                mnem_str = " ".join(mnem_tokens)
                out.append(f"{addr:>4}  {bytes_str:<11}  {mnem_str}")
            except Exception:
                out.append(line)
