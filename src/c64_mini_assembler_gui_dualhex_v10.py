...
class MiniAssembler:
    def __init__(self, spec: C6510Spec):
        ...
        # rows: (addr_hex, bytes_list, mnemonic_str, source_lineno)
    ...
    def assemble(self, text: str) -> Tuple[bytes,int,List[str]]:
        ...
        for ln in lines:
            ...
            if mnem in (".BYTE",".BYT",".TEXT",".ASC"):
                ...
                listing.append(f"{self.pc-len(bytes_here):04X}: " + " ".join(f"{b:02X}" for b in bytes_here) + f"    {mnem} {ln.operand or ''}")
                self.rows.append((f"{self.pc-len(bytes_here):04X}", bytes_here, f"{mnem} {ln.operand or ''}".strip(), ln.lineno))
                continue
            if mnem == ".WORD":
                bytes_here: List[int] = []
                for p in self._split_args(ln.operand):
            ...
            bytes_here = [opcode] + ob
            out.append(opcode); out.extend(ob)
            listing.append(f"{self.pc:04X}: " + " ".join(f"{b:02X}" for b in bytes_here) + f"    {mnem} {ln.operand or ''}")
            self.rows.append((f"{self.pc:04X}", bytes_here, f"{mnem} {ln.operand or ''}".strip(), ln.lineno))
            self.pc += 1+len(ob)
...
# ----------------- Code editor with line numbers, breakpoints, syntax, current-line highlight -----------------
...
class CodeEditor(QPlainTextEdit):
    def __init__(self, *args, **kwargs):
        ...
        self._breakpoints: Set[int] = set()  # store block numbers (0-based)
    ...
    def lineNumberAreaWidth(self) -> int:
        ...
        # extra space for breakpoint dot area + padding
    ...
    def mouseDoubleClickEvent(self, event):
        # if double-click lands in the gutter, toggle breakpoint
        if event.pos().x() < self.lineNumberAreaWidth():
            cursor = self.cursorForPosition(event.pos())
            block_num = cursor.blockNumber()
            if block_num in self._breakpoints:
                self._breakpoints.remove(block_num)
            else:
                self._breakpoints.add(block_num)
            self._lineNumberArea.update()
            # notify parent to refresh instruction view highlight
            mw = self.window()
...
class MainWindow(QMainWindow):
    ...
    # ------ Helpers to resolve operands to $ADDR for display in instr view ------
    ...
    # ------ View 2: instruction hex with bold header + breakpoint highlight ------
    def _refresh_hex_instr(self, rows: List[Tuple[str,List[int],str,int]]):
        ...
        def bytes_to_ascii(bseq):
            chars = []
            for b in bseq:
                if 32 <= b <= 126 and b not in (34,):  # printable ASCII except double quote
                    chars.append(chr(b))
                elif b == 34:  # quote -> escape as \"
                    chars.append('\\"')
                elif b == 0x20:
                    chars.append(' ')
                else:
                    # use PETSCII-ish placeholder dot for non-printables
                    chars.append('.')
            return "".join(chars)
        ...
        for addr_str, bytes_list, mnemonic_str, src_lineno in rows:
            ...
            if mnem_upper in (".TEXT", ".ASC"):
                # Split into string part (up to first 00) and trailing bytes (if any)
                data = list(bytes_list)
                # Find first zero for heuristic string terminator
                zero_idx = data.index(0x00) if 0x00 in data else len(data)
                str_bytes = data[:zero_idx]
                tail_bytes = data[zero_idx:] if zero_idx < len(data) else []

                # Make chunking over entire data (string + tail), 4 bytes per line
                full_data = data  # already includes tail
                base = int(addr_str, 16)

                # Precompute text for the string portion
                str_text = bytes_to_ascii(str_bytes)

                # Indices to slice string text per chunk (1 char per non-zero byte before zero)
                str_idx = 0
                total = len(full_data)
                off = 0
                while off < total:
                    chunk = full_data[off:off+4]
                    chunk_addr = (base + off) & 0xFFFF
                    bytes_tokens = " ".join(f"{b:02X}" for b in chunk)
                    # pad to keep columns readable
                    if len(chunk) < 4:
                        bytes_tokens = bytes_tokens.ljust(11)

                    # Build display mnemonic snippet
                    # Derive substring from str_text for the part that belongs to this chunk before the first zero
                    # Count how many bytes in this chunk are before the first zero (if any)
                    within_str = 0
                    for b in chunk:
                    ...
                    # Determine if this chunk contains the first zero terminator
                    contains_zero = any(b == 0x00 for b in chunk) and (off <= zero_idx < off+4 if zero_idx < len(full_data) else False)

                    # Decide quotes
                    # open quote on first line; close quote when we've emitted all string chars
                    open_quote = (off == 0)
                    close_quote = (str_idx >= len(str_text))

                    # Build mnemonic text
                    if open_quote:
                        seg_text = f'.TEXT "{segment}'
                    else:
                        # for continued lines, show .TEXT and continued segment start if any
                        seg_text = f'.TEXT "{segment}' if segment else ".TEXT"

                    if close_quote:
                        seg_text += '"'
                        # If this chunk also includes a 00 directly after string, append ", 0"
                        if contains_zero:
                            seg_text += ", 0"
            ...
            # Non-.TEXT/.ASC: previous behavior
            ...
            if mnem_upper in BRANCHES and len(bytes_list) >= 2:
                a = int(addr_str, 16)
