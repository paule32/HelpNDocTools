...
# ---- Bring your own spec ----
# expects: class C6510Spec with method get_opcode(mnemonic:str, mode:str)->int
...
class AsmHighlighter(QSyntaxHighlighter):
    ...
    def highlightBlock(self, text: str):
        ...
        # Label am Zeilenanfang (optional)
        m_label = self.re_label.match(code_part)
        if m_label:
            self.setFormat(m_label.start(1),
                           m_label.end(1) - m_label.start(1),
                           self.fmt_label)

        # Mnemonic (inkl. Direktiven wie .ORG/.BYTE)
        m_mnem = self.re_mnemonic.match(code_part)
        if m_mnem:
            self.setFormat(m_mnem.start(1),
                           m_mnem.end(1) - m_mnem.start(1),
                           self.fmt_mnemonic)
...
class MiniAssembler:
    def __init__(self, spec: C6510Spec):
        ...
        self.rows: List[Tuple[str, List[int], str, int]] = []  # instr view rows
...
# ----------------- PRG writer (Variant A: SYS to end-of-BASIC; payload immediately follows) -----------------
def build_prg_with_basic_autostart(payload: bytes, start_addr: int) -> bytes:
    LOAD_BASIC = 0x0801
    
    # Iterativ berechnen, da die Stellenanzahl der Dezimalzahl die Position beeinflusst
    while True:
        code_start = LOAD_BASIC  # Platzhalter
        dec = str(code_start).encode("ascii")
        sysline = bytes([0x9E]) + b" " + dec + b"\x00"  # SYS <dec>\0
        next_addr = LOAD_BASIC + 2 + 2 + len(sysline)   # Link + LineNo + content
        new_code_start = next_addr + 2                  # + two zero bytes (program end)
    ...
    prg.extend(basic)
    prg.extend(payload)                               # Direkt im Anschluss: ML-Payload
    return bytes(prg)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("C64 Mini Assembler — v6 (Variant A PRG)")
        self.resize(849, 600)
        ...
        self.hexview_main  = QTextEdit()
        self.hexview_main.setReadOnly(True)
        self.hexview_main.setLineWrapMode(QTextEdit.NoWrap)
        # --- C64 Pro Mono laden & setzen (für 1. View) ---
        self.hex_font_family = "C64 Pro Mono"  # Standardname
        self.hex_font = QFont(self.hex_font_family)
        self.hex_font.setStyleHint(QFont.Monospace)
        self.hex_font.setPointSize(10)         # Standard: 10pt
        
        # Optional: TTF-Datei explizit laden (falls der Font nicht systemweit installiert ist)
        # Pfad anpassen!
        try:
            font_id = QFontDatabase.addApplicationFont("./C64_Pro_Mono-STYLE.ttf")
            fams    = QFontDatabase.applicationFontFamilies(font_id)
            if fams:  # ersten Fontnamen aus der TTF verwenden
                self.hex_font_family = fams[0]
                self.hex_font = QFont(self.hex_font_family)
                self.hex_font.setPointSize(10)
                self.hex_font.setStyleHint(QFont.Monospace)
                self.hexview_main.setFont(self.hex_font)
        ...
        self.hexview_instr = QTextEdit()
        self.hexview_instr.setReadOnly(True)
        self.hexview_instr.setLineWrapMode(QTextEdit.NoWrap)
        self.hexview_instr.setFont(base_font)
        ...
        self.btn_run_vice = QPushButton("Start VICE")
        ...
        self.btn_run_vice.clicked.connect(self.on_run_click)
        ...
        try:
            ...
            pass
        try:
            self.org_combo.editTextChanged.connect(self.on_org_changed)
        except Exception:
            pass
        ...
        topbar.addWidget(self.btn_save_lst)
        topbar.addWidget(self.btn_run_vice)

        self.hex_font_size = QComboBox()
        self.hex_font_size.addItems(["10","9"])
        self.hex_font_size.setCurrentText("10")
        self.hex_font_size.currentTextChanged.connect(self.on_hex_font_size_changed)

        topbar.addWidget(QLabel("Hex-View Schrift:"))
        topbar.addWidget(self.hex_font_size)
    ...
    def on_hex_font_size_changed(self, s: str):
        try:
            pt = int(s)
        except ValueError:
            pt = 10
        self.hex_font.setPointSize(pt)
        self.hexview_main.setFont(self.hex_font)
        if self._last_payload is not None and self._last_org is not None:
            self._refresh_hex_main(self._last_org, self._last_payload)
            
    # entries: Liste von (pfad_zu_prg, disk_filename)
    def build_d64(self, image, diskname, diskid, entries):
        cmd = [
            'c1541',
            '-format', f'{diskname},{diskid}', 'd64', image,
            '-attach', image
        ]
    ...
    def on_run_click(self):
        self.build_d64('program.d64', 'HELLODISK', '01', [('program.prg','HELLO')])
    ...
    # ------ View 1: payload hex (addr + hex + text; bold header + rule) ------
    ...
    def _html_pre(self, inner: str) -> str:
        fam = self.hex_font_family.replace("'", "\\'")
        size_pt = self.hex_font.pointSize() or 10
        return (
            f"<pre style=\"font-family:'{fam}', monospace; "
            f"font-size:{size_pt}pt; margin:0;\">{inner}</pre>"
        )
    ...
    def _refresh_hex_main(self, base_addr: int, data: bytes):
        ...
        for i in range(0, len(data), 8):
            ...
            txt_raw = "".join(self._glyph_for_byte(b) for b in chunk)[:text_cols].ljust(text_cols)
            txt = htmlmod.escape(txt_raw)  # HTML-escapen!
            #lines.append(f"{addr:04X}  {left} | {right}  {txt}")
            #txt = "".join(self._glyph_for_byte(b) for b in chunk)[:text_cols].ljust(text_cols)
            #txt = "".join(petscii_text_char(b) for b in chunk)[:text_cols].ljust(text_cols)
            addr = base_addr + i
            lines.append(f"{addr:04X}  {left} | {right}  {txt}")
        html_str = self._html_pre("\n".join(lines))
        self.hexview_main.setHtml(html_str)
