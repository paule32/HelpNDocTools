...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("C64 Mini Assembler — Dual Hex Views (targets)")
    ...
    # ------ View 2: instruction hex (<=4 bytes per opcode + mnemonic + target/label) ------
    def _resolve_label(self, address: int) -> Optional[str]:
        # find first label matching the address
        for name, addr in self.assembler.symbols.items():
            if addr == address:
                return name
        return None
    ...
    def _refresh_hex_instr(self, listing: List[str]):
        BRANCHES = {"BPL","BMI","BVC","BVS","BCC","BCS","BNE","BEQ"}
        out = ["ADDR  BYTES(max4)    MNEMONIC                TARGET", "-"*72]
        for line in listing:
            try:
                ...
                # collect up to first 4 tokens that look like hex bytes
                byte_tokens = []
                mnem_start_idx = 0
                for idx, tok in enumerate(parts):
                    if re.fullmatch(r"[0-9A-Fa-f]{2}", tok):
                        byte_tokens.append(tok.upper())
                        # do not break early; we still need the first non-byte for mnemonic
                    else:
                        mnem_start_idx = idx
                        break
                if mnem_start_idx == 0:
                    # no mnemonic found (unlikely), fallback
                ...
                # Build bytes string (spaced, max 4 bytes shown)
                bytes_str = " ".join(byte_tokens[:4])

                # Compute target for branches and JSR/JMP abs
                target_info = ""
                if mnem in BRANCHES and len(byte_tokens) >= 2:
                    ...
                    target_info = f"$%04X" % target
                    if label: target_info += f" ({label})"

                elif mnem in {"JSR","JMP"}:
                    # Try to parse absolute address from bytes or operand
                    target = None
                    if len(byte_tokens) >= 3:
                        lo = int(byte_tokens[1],16); hi = int(byte_tokens[2],16)
                        target = (hi<<8) | lo
                    else:
                        # try parse operand like $C000 or label
                        op = operand_str.strip()
                        m = re.match(r"\$([0-9A-Fa-f]{1,4})", op)
                        if m:
                            target = int(m.group(1),16)
                        else:
                            # label lookup
                            if op in self.assembler.symbols:
                                target = self.assembler.symbols[op]
                    if target is not None:
                        label = self._resolve_label(target)
                        target_info = f"$%04X" % target
                        if label: target_info += f" ({label})"
                    elif mnem == "JMP" and "(" in operand_str:
                        # Indirect JMP: show pointer if bytes available
                        if len(byte_tokens) >= 3:
                            lo = int(byte_tokens[1],16); hi = int(byte_tokens[2],16)
                            ptr = (hi<<8) | lo
                            target_info = f"(*$%04X)" % ptr

                out.append(f"{addr_str:>4}  {bytes_str:<11}  {mnem} {operand_str:<18} {target_info}")
            except Exception:
                out.append(line)
